...
def test_s3_parallel_middle_pipeline_execution(
    sfn_client, sfn_arn, sfn_payload, s3_client
):
    """Test that the s3-parallel-middle pipeline executes successfully.

    This pipeline runs prepare_data (lambda) first, which writes subdirectories
    to the intermediate bucket. Then the parallel block discovers those
    directories and fans out to process_item for each one.
    """
    ...
    try:
        ...
        # Verify process_item wrote output for each discovered item
