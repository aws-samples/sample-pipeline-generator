...
"""Tests for the prepare_data Lambda step."""
...
class TestHandler:
    ...
    def _env(self, monkeypatch):
        monkeypatch.setenv("STEP_NAME", "prepare_data")
        monkeypatch.setenv("INTERMEDIATE_BUCKET", "test-intermediate")

    @patch("main.s3")
    def test_writes_subdirectories_to_intermediate_bucket(self, mock_s3):
        event = {"SFN_EXECUTION_ID": "exec-001"}

        result = handler(event, None)

        assert result["status"] == "success"
        assert result["subdirectories"] == ["batch-a", "batch-b", "batch-c"]
        # One S3 write per subdirectory.
        assert mock_s3.put_object.call_count == 3
        keys = [c[1]["Key"] for c in mock_s3.put_object.call_args_list]
        assert keys == [
            "exec-001/prepare_data/batch-a/data.json",
            "exec-001/prepare_data/batch-b/data.json",
            "exec-001/prepare_data/batch-c/data.json",
        ]
    ...
    def test_info_logs_do_not_contain_raw_step_or_execution_input(
        ...
        event = {
            ...
            "STEP_PREVIOUS": {"token": sensitive_step_value},
