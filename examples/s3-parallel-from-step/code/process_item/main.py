...
Receives MAP_ITEM (the discovered S3 prefix from the intermediate bucket)
and writes a marker file to confirm processing.
...
logger = Logger(service="s3-parallel-from-step-process-item")
...
def handler(event, context):
    ...
    # map_item already contains the full prefix including execution_id and
    # source step (e.g. "<exec_id>/prepare_data/batch-a/").
    # Extract just the item-specific suffix (everything after <exec_id>/<step>/)
    # and write under this step's namespace.
    parts = map_item.strip("/").split("/", 2)
    item_suffix = parts[2] if len(parts) > 2 else parts[-1]
    output_key = f"{sfn_execution_id}/{step_name}/{item_suffix}/processed.json"
    s3.put_object(
